...
function loadAlphabet(lang) {
	...
	symbolCor["<".charCodeAt(0)]={x:64, y: 0};
	symbolCor[">".charCodeAt(0)]={x:64, y: 10};
	symbolCor["%".charCodeAt(0)]={x:64, y: 20};
}
function createText(w,h, x,y, text, size, dist) {
	...
	for (let i=0;i<arr.length;++i) {
		...
		if (source!=undefined) context.drawImage(imgText, source.x, source.y, 8, 10, dx+dist, dy, 8*size, 10*size);
		dx+=(8*size);
...
function drawText_c(context, x,y, text, size, dist) {
	text=text.toString();
	let arr = text.split("");
	let dy = y;
	let dx = x;
...
function drawAnimText(obj) {
	...
	for (let i=0;i<obj.cadr;++i) {
		let symbol=obj.text[i];
		let code=symbol.charCodeAt(0);
		let source=symbolCor[code];
		if (source!=undefined) ctxInterface.drawImage(imgText, source.x, source.y, 8, 10, dx, dy, 8*obj.size, 10*obj.size);
		dx+=(8*obj.size+obj.dist);
		if (symbol=="\n") {
			dy+=10*obj.size;
			dx=obj.x;
		}
